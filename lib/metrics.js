...
class Metrics {
  ...
  complexityRepoPerf(paths) {
    ...
    const metrics = this.complexityPathsPerModule(filePaths);
    const connections = {};
    ...
    return {
      performance: metrics,
      performanceConnections: connections
    }
  }
  ...
  connections(sourcePaths, testPaths) {
    ...
    testPaths.forEach((path) => {
      const fixedModuleName = path.split('/').at(-1).replace(/[\.\-_]?[tT]est[s\-_]?|[\.\-_]?[sS]pec[s\-_]?/, '');
    ...
    for (let i = 0; i < paths.length; i++) {
      ...
      try{
        ...
        Object.entries(metrics).forEach(([key, value]) => {
          if (Array.isArray(value) && value.length > 0) metrics[key] = this.valuesFromArr(value);
        });
      }
      catch(e) {
        console.error(e);
