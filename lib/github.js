class GitHub {
  constructor() {
    ...
    this.maxPR = -1;
    this.repoDownload = require('download-git-repo');
  ...
  async getRepository(owner, repo) {
    try {
      ...
      console.log(e);
      throw 'Error getting GitHub repo';
    }
  }
  ...
  async searchFileInRepo(repo, filename, language) {
    const response = await this.octokit.request('GET /search/code', {
      q: `{+repo:${repo}+filename:${filename}+extension:.${language}`
    });
    return response;
  }
  ...
  async downloadGitRepo(repo, destination) {
    return new Promise((resolve, reject) => {
      this.repoDownload(repo, destination, {}, (err) => {
        if (err) {
          console.error(err);
          reject(false);
        }
  ...
  async getCommitsForFile(repo, owner, file) {
    const data = await this.octokit.paginate('GET /repos/{owner}/{repo}/commits', {
      owner: owner,
      repo: repo,
      path: file,
      per_page: 100,
  ...
  async getPrsForCommit(repo, owner, commit) {
    const data = await this.octokit.paginate('GET /repos/{owner}/{repo}/commits/{commit_sha}/pulls', {
      owner: owner,
      repo: repo,
      commit_sha: commit,
      per_page: 100,
