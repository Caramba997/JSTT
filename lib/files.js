...
class Files {
  ...
  async raw(type, file, vars) {
    ...
    return vars && vars[0] === 'pavel' ? await this.aws.download(path) : await fsP.readFile(path, { encoding:'utf8', flag:'r' });
  }
  ...
  async json(type, file, vars) {
    ...
    const data = vars && vars[0] === 'pavel' ? await this.aws.download(path) : await fsP.readFile(path, { encoding:'utf8', flag:'r' });
  ...
  async write(type, file, data, vars, raw = false) {
    ...
    const path = this.path(type, file, vars);
    if (vars && vars[0] === 'pavel') {
      await this.aws.upload(path, strData);
    }
    else {
      await fsP.writeFile(path, strData);
    }
    return true;
  }
